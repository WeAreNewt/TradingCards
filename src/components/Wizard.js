...
import { ClockIcon, TicketIcon, CreditCardIcon } from '@heroicons/react/outline'
import { RadioGroup } from '@headlessui/react'
import PreviewCard from './PreviewCard'
import { giveApproval, stakeNft } from '../utils/helpers.js'
...
export default function Wizard(props) {
  const { wallet, collections } = props
  ...
  return (
    <>
      <main className="lg:col-span-9 xl:col-span-6">
        ...
        {currentStep === 2 && (
          <div
            className="bg-white px-4 py-10 rounded-lg shadow-inner"
            style={{
                  ...
                  ${checked ? 'bg-white shadow-md' : 'bg-white opacity-80'}
                    relative rounded-lg cursor-pointer flex focus:outline-none `
                      ...
                      >
                        {({ active, checked }) => (
                          <>
                            <div className="flex items-center px-5 py-4 justify-between w-full cursor-pointer">
        ...
        {currentStep === 3 && (
          <div className="bg-white px-8 py-6 rounded-lg space-y-8 ">
            <div className="w-full flex flex-row space-between items-center">
              <div className="flex flex-col">
                ...
                <p className="mt-1 mb-2 text-sm text-gray-500">
                  In order to stake your nft you first need to give the trading
                  card contract approval to transfer your nft.
                </p>
              </div>
              {!isApproved && (
                <button
                  type="button"
                  onClick={() => approve()}
                  className="inline-flex flex-grow-1 max-w-16 h-12 items-center text-center justify-center mt-2 px-3 py-3 border border-transparent text-md leading-4 font-medium rounded-md text-orange-400 bg-orange-100 hover:bg-orange-200 "
                >
                  {isApproving && (
                    <svg
      ...
      <aside className="hidden xl:block xl:col-span-4">
        <div className="sticky top-4 space-y-4">
          ...
          <div className="width-full px-6 bg-gray-100 rounded-lg">
            <PreviewCard nft={selectedNft} rarity={selected} />
          </div>
        </div>
      </aside>
    </>
