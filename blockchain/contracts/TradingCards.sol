...
contract TradingCards is ERC721Enumerable, Ownable {
    ...
    function stakeNft(
        ...
        require(
            ...
            "Target nft is not whitelisted for staking"
        );
        require(
            hasNFTBeenStaked[nftContract][nftId] == false,
            "Target nft has already been staked before"
        );
    ...
    function unstakeNft(uint256 cardId) external {
        ...
        require(
            block.timestamp >= targetCard.timestamp + targetCard.duration,
            "Staking period not over yet"
        );
        require(targetCard.inVault == true, "Nft already unstaked");
        require(
            targetCard.owner == address(msg.sender),
            "Only the original nft staker can unstake"
        );

        targetCard.inVault = false;
        IERC721(targetCard.tokenContract).transferFrom(
            address(this),
            msg.sender,
            targetCard.tokenId
        );
    ...
    function buyTradingCard(uint256 cardId) external payable onlyOpenWindow {
        ...
        require(
            block.timestamp < (targetCard.timestamp + targetCard.duration),
            "Card minting window has passed"
        );
        require(
            targetCard.copies < targetCard.supply,
            "Card has reached supply cap"
        );
        require(
            msg.value == targetCard.price,
    ...
    function getCardInfo(uint256 cardId)
        external
        view
        returns (StakedNft memory cardInfo)
    {
